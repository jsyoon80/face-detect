...
#st.title("AiVisionPro")



st.title("컴퓨터 머신비전 딥러닝 통한 ")
st.title("얼굴 인식 및 검출 프로그램")
img_file_buffer = st.file_uploader("이미지 파일을 선택하세요.", type=['jpg', 'jpeg', 'png'])
...
if img_file_buffer is not None:
    ...
    placeholders[0].text("입력 이미지")

    # Create a Slider and get the threshold from the slider.
    conf_threshold = st.slider("0.00~1.00까지 보다 높은 신뢰도를 조절해 보세요 ", min_value=0.0, max_value=1.0, step=.01, value=0.5)
